# 🚀 Deployment Guide: The 3-Week Playbook

Deploy a new customer platform end to end in 15 working days — infrastructure, Open edX, MFEs, integrations, content, go-live. Every phase has a verification gate.
...
6. **Day 6 — Install Tutor** (Ulmo release) on the platform EC2 via the bootstrap scripts; configure domain, DB connections (RDS MySQL, DocumentDB). *Verify:* `tutor local status` → services up; `curl https://<domain>/heartbeat` → `OK`.
7. **Day 7 — Open edX configuration:** site config, theming, user creation, Studio access. *Verify:* login at `/login`; Studio reachable at the studio endpoint.
