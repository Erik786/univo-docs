...
- On successful sign-in the MFE:
  ...
  2. For privileged roles, additionally establishes a session with the course-engine LMS backend (`REACT_APP_BASE_URL`) via its CSRF + `login_session` endpoints.
...
| Misc | react-multi-carousel, `querystring` (form-encoding for the engine login POST) |
...
| `REACT_APP_BASE_URL` | Course-engine base URL, CSRF + `login_session` endpoints |
